...
var overlayOff = function off() {
  document.getElementById('overlayId').style.display = 'none';
};

var overlayOn = function on() {
...
var postGameFailure = function postGameFailure() {};
...
var statsFailure = function statsFailure() {
...
module.exports = {
  ...
  postGameFailure: postGameFailure,
...
var show = function show(playerId) {
  return $.ajax({
    url: config.apiOrigin + '/games',
    method: 'GET',
    headers: {
...
var getStats = function getStats() {
  ...
  var playerId = store.user.id;
  api.show(playerId).then(ui.statsSuccess).catch(ui.statsFailure);
};
...
var onSignOut = function onSignOut(event) {
  event.preventDefault();
  api.signOut().then(ui.signOutSuccess).then(game.onReplay).then($('#result').text('WELCOME TO THE GAME GRID')).catch(ui.signOutFailure);
};

var onPasswordChange = function onPasswordChange(event) {
  var data = getFormFields(this);
  event.preventDefault();
  api.passwordChange(data).then(ui.changePasswordSuccess).catch(ui.changePasswordFailure);
};
...
var onGameStart = function onGameStart(event) {
  ...
  game.onReplay(event);
  api.postGame().then(ui.postGameSuccess).catch(ui.postGameFailure);
  $('#start-game').hide();
  $('#replayGame').show();
  $('#ai-game').show();
  $('#replayAiGame').hide();
...
var onReplayStart = function onReplayStart(event) {
  game.onReplay(event);
  getStats();
  event.preventDefault();
  api.postGame().then(ui.postGameSuccess).catch(ui.postGameFailure);
...
var onAiGameStart = function onAiGameStart(event) {
  event.preventDefault();
  $('.col-xs-4').unbind('click');
...
var onAiReplayStart = function onAiReplayStart(event) {
  game.onAiReplay(event);
  getStats();
  event.preventDefault();
  api.postGame().then(ui.postGameSuccess).catch(ui.postGameFailure);
...
module.exports = {
  ...
  getStats: getStats,
  onAiGameStart: onAiGameStart,
  onAiReplayStart: onAiReplayStart
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
var aiCellInput = function aiCellInput(squareSelected) {
  if (player === 0) {
...
var cellInput = function cellInput(squareSelected) {
  if (player === 0) {
    squareSelected.addClass('pOne').text('X');
    if (checkWin('pOne') === true) {
      $('#result').text('PLAYER ONE WINS');
...
var checkSquare = function checkSquare(squareSelected) {
...
var checkAiSquare = function checkAiSquare(squareSelected) {
  if (squareSelected.hasClass('pOne') || squareSelected.hasClass('pTwo')) {
    $('#result').text('INVALID MOVE');
  } else {
...
var addHandlers = function addHandlers() {
  $('.col-xs-4').on('click', function (event) {
    ...
    api.updateGame(data).then(ui.patchGameSuccess).catch(ui.patchGameFailure);
  });
};

var addAiHandlers = function addAiHandlers() {
  $('.col-xs-4').on('click', function (event) {
    var squareSelected = $(this);
    createBoard(this.id);
...
var onAiReplay = function onAiReplay(event) {
  $('.col-xs-4').removeClass('pOne');
  $('.col-xs-4').removeClass('pTwo');
  $('.col-xs-4').empty();
  $('#result').empty();
...
module.exports = {
  ...
  cellInput: cellInput,
  checkWin: checkWin,
  emptySquareTotal: emptySquareTotal,
  showTie: showTie,
  onReplay: onReplay,
  createBoard: createBoard,
  endClick: endClick,
  pOneSymbol: pOneSymbol,
  addAiHandlers: addAiHandlers,
  onAiReplay: onAiReplay
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
